...
class MicroMouseController:
    ...
    def __init__(
        ...
        try:
            self.maze.load_maze(pd.read_csv("bin/mazes/maze_data 2024-12-23, 121810.csv"))
        except:
            # for jupyter
            self.maze.load_maze(pd.read_csv("mazes/maze_data 2024-12-23, 121810.csv"))
    ...
    def update(
        ...
        try:
            ...
            try:
                ...
                command= ["ac", MicroMouseMaze.check_intersection_with_curve([(-1)*__x_rel, __y_rel], math.radians(angle))-hf.map_value(self.position.y, 606, 24, 0, 15), False, None]
...
class customHumanControlled(HumanControlled):
    __log_file_name: str = time.strftime("bin/mm_data/micromouse_data %Y-%m-%d, %H%M%S.csv")
    ...
    def event_handler(self, events):
        ...
        self.mouse.event_handler(events)
    ...
    def update(self, time_step: float, events):
        ...
        if self.current_command[0]=="ac":
            ...
            # self.set_acceleration([(random.choice(self.__acc_values))*(1-(correction/100))*0.3, 0.0])
            self.set_acceleration([(random.choice(self.__acc_values))*0.6, 0.0])
        ...
        data = {
            ...
            'LIDAR': [self._sensors[i].distance for i in range(len(self._sensors))],
        }
...
def get_rel_pos(position:pg.Vector2):
    return pg.Vector2(
        hf.map_value(position.x, 1102, 515, 0, -15),
        hf.map_value(position.y, 606, 24, 0, 15),
    ) #* throws type error if calculate_error_and_round returns None
...
if __name__ == "__main__":
    ...
    robots = [
        customHumanControlled(
            ...
            sensors=[
                ...
                customLIDARSensor(angle=-30),
                customLIDARSensor(angle=-19),
    ...
    simulator = MazeSim(
        ...
        overlays=[
            ...
            lambda:
            f"rel position = {get_rel_pos(hf.round_vec_2d(robots[0].get_position(), 3))}\n",
        ],
    )
